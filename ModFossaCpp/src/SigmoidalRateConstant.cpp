...
#include <SigmoidalRateConstant.h>
...
SigmoidalRateConstant::SigmoidalRateConstant(string name, double a,
...
double SigmoidalRateConstant::getRate(StateOfTheWorld* state_of_the_world) {
    if (!name_initialized) {
...
string SigmoidalRateConstant::getName() const {
    if (!name_initialized) {
        throw std::runtime_error("name is not initialized");
    }
...
void SigmoidalRateConstant::setName(string name) {
    if (name.empty()) {
        throw std::runtime_error("Error: name is empty string");
    }
